...
$nissan       = mysqli_query($koneksi, "SELECT merk FROM tb_mobil WHERE merk = 'Nissan' ");


$toyota      = mysqli_query($koneksi, "SELECT merk FROM tb_mobil WHERE merk = 'Toyota' ");
?>
...
<fieldset>
    ...
    <table width="100%" border="1px" style="border-collapse:collapse;">
        ...
        if($cari_iklan){
            ...
            $sql = mysqli_query($koneksi, "select * from tb_mobil where merk like '%$inputan_pencarian%' or type like '%$inputan_pencarian%' order by status ASC") or die (mysqli_error($koneksi));
            } else{
                $sql = mysqli_query($koneksi, "select * from tb_mobil order by status ASC") or die (mysqli_error($koneksi));
            }
        } else {
            $sql = mysqli_query($koneksi, "select * from tb_mobil order by status ASC") or die (mysqli_error($koneksi));
        }
        ...
        ?>
            <tr>
                ...
                <td align="center">
                    <a onclick="return confirm('Yakin ingin menghapus iklan?')" href="?page=iklan_manajemen&action=hapus&kdmobil=<?php echo $data['kode_mobil']; ?>"><button>Hapus</button></a>
                    <a href="?page=iklan_manajemen&action=terjual&kdmobil=<?php echo $data['kode_mobil']; ?>"><button>Terjual</button></a>
                    <a href="?page=iklan_manajemen&action=verifikasi&kdmobil=<?php echo $data['kode_mobil']; ?>"><button>Verifikasi</button></a>
                </td>
            </tr>
        <?php
        }
